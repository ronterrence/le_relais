    ...
    if N8N_URL:
        try:
            ...
            if isinstance(data, list) and data and isinstance(data[0], dict):
                answer = data[0].get("response") or data[0].get("answer") or data[0].get("message")
                if answer:
                    return str(answer)
        except requests.RequestException:
            st.toast("Webhook n8n indisponible: reponse locale affichee.")
        except ValueError:
            st.toast("Reponse n8n non JSON: reponse locale affichee.")

    if not organismes:
        return (
            f"Je n'ai pas encore de resultat pour le code postal {postal_code}. "
            "L'interface est prete a interroger l'API ou le webhook n8n des que la configuration repond."
        )

    names = ", ".join(org.name for org in organismes[:3])
    return (
        f"Pour votre besoin ({need or 'a preciser'}) autour du code postal {postal_code}, "
        f"j'ai trouve {len(organismes)} organisme(s): {names}. "
        "Vous pouvez consulter les fiches a droite pour verifier l'adresse, le type de service et les coordonnees."
    )
